...
def Gen_mock_gal(Nsample     = 1000, 
    ...
    zprime = z_posterior
    #-----------
    zprime = z_posterior # it is the redshift at which we compute the density function of sfr-mass-z when z<0.2
    ...
    def weight_func(mstar=None, z=None, weight=None) :
        ...
        # Check if metallicity weighting is requested
        use_Z = False
        use_iZ = False
        if weight is not None and '_Z' in weight:
            use_Z = True
            base_weight = weight.replace('_Z', '')
        elif weight is not None and '_iZ' in weight:
            use_iZ = True
            base_weight = weight.replace('_iZ', '')    
        else:
            base_weight = weight

        if base_weight=='SFR' and (mode!='nn') :
            log_sfr = utls.SFMS(Mstar=mstar, z=z, sfr_ref=sfr_ref, mode=mode, sample_size=sample_size, posterior=posterior)
            val = 10**log_sfr
            if use_Z:
                Z = 10**utls.get_metallicity(np.log10(mstar), log_sfr)
                return val * Z
            if use_iZ:
                Z = 10**utls.get_metallicity(np.log10(mstar), log_sfr)
                return val / Z
        ...
        elif base_weight=='mass' :
            val = mstar/np.max(mstar) #weight by stellar mass
            if use_Z:
                # Use ridge SFR for metallicity calculation when weighting by mass
                log_sfr = utls.SFMS(Mstar=mstar, z=z, sfr_ref=sfr_ref, mode='ridge', sample_size=sample_size, posterior=False)
                Z = 10**utls.get_metallicity(np.log10(mstar), log_sfr)
                return val * Z
            if use_iZ:
                # Use ridge SFR for metallicity calculation when weighting by mass
        ...
        elif base_weight=='SFR' and (mode=='nn'):
            ...
            val = 10**sfr_sample #sampled from the posterio distribution
            if use_Z:
                Z = 10**utls.get_metallicity(np.log10(mstar), sfr_sample)
                return val * Z
            if use_iZ:
                Z = 10**utls.get_metallicity(np.log10(mstar), sfr_sample)
                return val / Z
    ...
    if (space_dist=='delta') :
        ...
        for i, zi in zip(range(len(z_arr)), z_arr) :
            ...
            cdf_pz = cumulative_trapezoid(p_z, np.log10(mstar), initial=0) #integrate over logm so that the cdf has units of counts times the units of the weight function
            norm   = cdf_pz[-1]  
...
def mock_realization(zbins = [0.,0.3, 0.7], 
        ...
        if space_dist=='delta' :
            log_mstar_samples, _, z_values = Gen_mock_gal(Nsample=Nsample, z_arr=zgal, plot_cdf=False, weight=weight,\
                                                                    mfunc_ref=mfunc_ref, mfunc_mstar0=mfunc_mstar0, \
                                                                        ...
                                                                        space_dist=space_dist, z_min=z_min, z_max=z_max, z_posterior=z_posterior) 
            ...
            for i in range(len(zgal)):
                ...
                for zij in zi_bins:
                    log_mstar_sample, _, _ = Gen_mock_gal(Nsample=Nsubsample, z_arr=zij, plot_cdf=False, weight=weight,\
                                                                    mfunc_ref=mfunc_ref, mfunc_mstar0=mfunc_mstar0, \
                                                                        mfunc_slope=mfunc_slope, sfr_ref=sfr_ref, mode=no_post_mode, sfr_sampling=False,\
                                                                        space_dist='delta', z_min=z_min, z_max=z_max, plot_pdf=False, z_posterior=z_posterior)
    ...
    if data_source in ['Sharma_only', 'Sharma_full', 'CCSN', 'CCSN_Ibc'] : 
        if 'Sharma' in data_source :
        ...
        elif(data_source=='CCSN') :
            frbdata_mstar, frbdata_ztransient = utls.load_CCSN_host_galaxies() # it returns the stellar masses and redshifts of CCSN host galaxies
                                                                                # customized for testing purposes. Also frb is just there for compatibility, its ccsn data
        elif(data_source=='CCSN_Ibc') :
            frbdata_mstar, frbdata_ztransient = utls.load_CCSN_Ibc_host_galaxies() # it returns the stellar masses and redshifts of CCSN host galaxies of type Ibc
                                                                                # customized for testing purposes. Also frb is just there for compatibility, its ccsn data
    else : 
        raise Exception(f"The FRB host data source : <{data_source}> is not available. Try again.")
    ...
    for i in range(len(zgal)) : # iterate over all different redshift bins
        ...
        if plot_cdf_ridge :
            ...
            if space_dist != 'delta': _, ind_magcut = magnitude_cut(log_mstar=sample, z=zgal[i], rmag_cut=rmag_cut, plot=False, z_values=z_values[i], space_dist=space_dist)
            else : 
                try :
            ...
            for n in tqdm(range(nlines), desc=f"Sampling posterior for {zbins[i]} $<$ z $\leq$ {zbins[i+1]}") : 
                if space_dist == 'delta':
                    post_sample = Gen_mock_gal(Nsample=Nsample, z_arr=zgal[i], plot_cdf=False, weight=weight, mfunc_ref=mfunc_ref, mfunc_slope=mfunc_slope, mfunc_mstar0=mfunc_mstar0, \
                                                        sfr_ref=sfr_ref, mode=mode, posterior=True, space_dist=space_dist, z_min=z_min, z_max=z_max, z_posterior=z_posterior)[0]
                    ...
                    for zij in zij_bins[i] :
                        post_subsample = Gen_mock_gal(Nsample=Nsubsample, z_arr=zij , plot_cdf=False, weight=weight, mfunc_ref=mfunc_ref, mfunc_slope=mfunc_slope, mfunc_mstar0=mfunc_mstar0, \
                                                        sfr_ref=sfr_ref, mode=mode, posterior=True, space_dist='delta', z_min=z_min, z_max=z_max, z_posterior=z_posterior)[0]
                    ...
                    if space_dist == 'delta' :
                        post_sample = Gen_mock_gal(Nsample=Nsample, z_arr=zgal[i], plot_cdf=False, weight=weight, mfunc_ref=mfunc_ref, mfunc_slope=mfunc_slope, mfunc_mstar0=mfunc_mstar0, \
                                                        sfr_ref=sfr_ref, mode=mode, sfr_sampling=sfr_sampling, completeness_handling=completeness_handling, \
                                                             sigma_norm=sigma_norm, posterior=True, posterior_interp=posterior_interp, \
                                                                lgsfr_grid=p_logsfr_arr, space_dist=space_dist, z_min=z_min, z_max=z_max, z_posterior=z_posterior)[0]
                        ...
                        for zij in zij_bins[i] :
                            ...
                            post_subsample = Gen_mock_gal(Nsample=Nsubsample, z_arr=zgal[i], plot_cdf=False, weight=weight, mfunc_ref=mfunc_ref, mfunc_slope=mfunc_slope, mfunc_mstar0=mfunc_mstar0, \
                                                        sfr_ref=sfr_ref, mode=mode, sfr_sampling=sfr_sampling, completeness_handling=completeness_handling, \
                                                             sigma_norm=sigma_norm, posterior=True, posterior_interp=posterior_interp, \
                                                                lgsfr_grid=p_logsfr_arr, space_dist='delta', z_min=z_min, z_max=z_max, z_posterior=z_posterior)[0]
                    ...
                    if space_dist == 'delta' :
                        ...
                        for zij in zij_bins[i] :
                            ...
                            post_subsample, sublog_sfr, sublog_sfr_mode = Gen_mock_gal(Nsample=Nsubsample, z_arr=zij, plot_cdf=False, weight=weight, mfunc_ref=mfunc_ref, mfunc_slope=mfunc_slope, mfunc_mstar0=mfunc_mstar0, \
                                                        sfr_ref=sfr_ref, mode=mode, sfr_sampling=sfr_sampling, completeness_handling=completeness_handling, \
                                                             sigma_norm=sigma_norm, posterior=True, posterior_interp=posterior_interp, lgsfr_grid=p_logsfr_arr, \
                                                                space_dist='delta', z_min=z_min, z_max=z_max, z_posterior=z_posterior)
