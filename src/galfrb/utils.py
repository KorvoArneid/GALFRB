...
def get_metallicity(logm, logsfr):
    """
    ...
    m = logm - 10.0
    s = logsfr
    # Mannucci et al. 2010 Eq. 4
    OH = 8.90 + 0.37*m - 0.14*s - 0.19*(m**2) + 0.12*m*s - 0.054*(s**2)
    return OH 
...
def interpolate_prob(prob_arr=None, z_arr=None, logm_arr=None, logsfr_arr=None, ztarget=1., plot=False):
    ...
    # Use RectBivariateSpline instead of interp2d (which is removed in SciPy 1.14+)
    # interp2d(x, y, z) corresponds to RectBivariateSpline(x, y, z.T)
    # We wrap it to maintain the (ny, nx) output shape behavior of interp2d
    rbs = RectBivariateSpline(logsfr_arr, logm_arr, posterior.T)
    interpolator = lambda x, y: rbs(x, y).T
...
def load_CCSN_host_galaxies():
    """
    Load CCSN host galaxies data from Schulze et al. (2021)
    ...
    data9 = pd.read_csv(rltv_pth + '/data/schulze_table9.csv')
    data1 = pd.read_csv(rltv_pth + '/data/schulze_table1.csv')
    data1 = data1[data1['ID'].isin(data9["ID"].unique())] # select only hosts in table 9, from table 1
    data1.drop_duplicates(subset=['ID'], inplace=True)
    data1 = data1[['ID', 'logM', 'z']]
    print("CCSN host galaxies data are loaded")
    return np.power(10, data1['logM'].values), data1['z'].values

def load_CCSN_Ibc_host_galaxies():
    """
    Load CCSN Ibc host galaxies data from Schulze et al. (2021)
    ...
    data1 = data1[data1['ID'].isin(data9["ID"].unique())] # select only hosts in table 9, from table 1
    data1.drop_duplicates(subset=['ID'], inplace=True)
    data1 = data1[['ID', 'logM', 'z', 'Type']]
    data1 = data1[data1['Type'].isin(['SNIb', 'SNIc'])] # select only Ibc types
    print("CCSN Ibc host galaxies data are loaded")
    return np.power(10, data1['logM'].values), data1['z'].values
